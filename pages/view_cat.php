...
$data = $catObject->SelectCat();

?>
<!-- Page -->
<form method="post" action="">
    <div class="page-header">
        <h1 class="page-title">عرض التصنيفات</h1>
        <ol class="breadcrumb">
        ...
        <div class="page-header-actions">
            <a class="btn btn-sm btn-icon btn-primary btn-round" data-toggle="tooltip"
               data-original-title="اضافة جديد" href="index.php?page=manage_cat&action=insert">
    ...
    <div class="page-content">
        <?php
        $catObject->ActivatedRecords();
        $catObject->DeactivatedRecords();
        $catObject->DeleteRecords();
        ?>
        <!-- Panel Table Tools -->
        <div class="panel">
            <header class="panel-heading">
                <h3 class="panel-title">عرض التصنيفات</h3>
            </header>
                    ...
                    <?php } else {
                        foreach ($data as $val) { ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    if (empty($catObject->GetAttr("name", $val['parent_id']))) {
                                        echo "رئيسي";
                                    } else {
                                        echo $catObject->GetAttr("name", $val['parent_id']);
                                    }
                                    ?>
                                </td>
                                <td><?= $val['username'] ?></td>
                                ...
                                <td class="text-center">
                                    <a class="btn btn-sm btn-icon btn-primary btn-round" data-toggle="tooltip"
                                       ...
                                       href="index.php?page=manage_cat&action=update&id=<?= $val['id'] ?>">
